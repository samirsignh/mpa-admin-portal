            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">SAROD List</h4>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="example" class="display text-center table-bordered">
                                ...
                                <tbody>
                                    @if (count($sarod_lists) > 0)
                                    @foreach ($sarod_lists as $key => $list)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $list->title }}</td>
                                        <td>
                                            <a class="sarod_file" target="_blank"
                                                href="{{ asset($list->sarod_document) }}"><i
                                                    class="fa-solid fa-file-pdf"></i></a>
                                        </td>
                                        <td>
                                            <a class="edit-icon" href="{{ route('editSarod', $list->id) }}"><i
                                                    class="fa-solid fa-pen-to-square"></i></a>
                                            <a class="delete-icon" href="{{ route('deleteSarod', $list->id) }}"><i
...
<div class="modal fade add-user-type" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <form action="{{ route('uploadSarodFile') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title">SAROD Details</h5>
